...
// Busca si un anillo tiene una talla específica, directamente en el
// texto de productos.talla (sin convertir todo a listas de números).
// Ej: buscar 6.75 arma el patrón "6 Y 3/4" y revisa si aparece en el
// texto, aunque venga acompañado de otras tallas: "5 Y 3/4 - 6 Y 3/4 - 7 Y 1/2"
const FRACCIONES = { 0.25: [1, 4], 0.5: [1, 2], 0.75: [3, 4] };

const construirPatronTalla = (decimal) => {
  const entero = Math.floor(decimal);
  const frac = Math.round((decimal - entero) * 100) / 100;

  if (frac === 0) {
    // Talla entera: no debe estar seguida de una fracción
    // (para que "7" no confunda con "7 Y 1/2")
    return new RegExp(`\\b${entero}\\b(?!\\s*(?:Y\\s*)?\\d\\s*/)`, "i");
  }

  const fraccion = FRACCIONES[frac];
  if (!fraccion) {
    // Valor de talla no estándar (ej. URL armada a mano): no debe
    // romper la búsqueda, simplemente no encuentra nada.
    return /(?!)/;
  }

  const [num, denom] = fraccion;
  return new RegExp(
    `\\b${entero}\\s*(?:Y\\s*)?${num}\\s*/\\s*${denom}\\b`,
    "i",
  );
};

const tieneTallaAnillo = (texto, decimal) => {
  if (!texto) return false;
  const normalizado = texto.replace("//4", "3/4");
  return construirPatronTalla(decimal).test(normalizado);
};
