...
// Devuelve un string YYYY-MM-DD para evitar desfases por zona horaria
const calcularFechaPago = (año, mes, diaPago) => {
  // mes viene en base 0 (0 = enero). Normalizar si se pasó de diciembre.
  let añoFinal = año;
  let mesFinal = mes;
  if (mesFinal > 11) {
    mesFinal = 0;
  ...
  const ultimoDia = new Date(añoFinal, mesFinal + 1, 0).getDate();
  const diaReal = Math.min(diaPago, ultimoDia);
  // Construir string YYYY-MM-DD manualmente (sin toISOString para no desfasar)
  const mesStr = String(mesFinal + 1).padStart(2, "0");
  const diaStr = String(diaReal).padStart(2, "0");
  return `${añoFinal}-${mesStr}-${diaStr}`;
};
...
export const registrarPago = async (creditoId, { monto, tipo, notas }) => {
  ...
  // Calcular próximo pago (el mes siguiente al actual, en el día de pago)
  ...
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      estado: nuevoEstado,
      fecha_proximo_pago: nuevoSaldo > 0 ? proximoPago : null,
