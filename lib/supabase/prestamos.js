...
// Agregar una joya olvidada a un préstamo que ya está activo.
// Descuenta el stock igual que si se hubiera incluido desde el inicio.
...
// Quitar una joya que se agregó por error a un préstamo (nunca llegó
// a salir físicamente). Devuelve el stock y borra el registro por
// completo, sin dejarlo marcado como "devuelto" porque nunca se prestó
// de verdad.
...
// Regresa a "prestado" un ítem que se había marcado por error como
// vendido o devuelto. Revierte el stock que se le había devuelto en ese
// momento, y reabre el préstamo si ya se había cerrado.
export const revertirItemPrestamo = async (idDetalleItem) => {
  const { data: item, error: itemError } = await supabase
    .from("prestamos_detalle")
    .select("id, id_producto, cantidad, estado_item, id_prestamo")
    .eq("id", idDetalleItem)
  ...
  if (item.estado_item === "prestado") {
    throw new Error("Este ítem ya está prestado");
  }
  ...
  if (producto.stock < item.cantidad) {
    throw new Error(
      "El stock de este producto ya cambió desde que se marcó como vendido/devuelto (puede que ya se haya vendido de nuevo). Revisa el inventario antes de continuar.",
    );
  }
  ...
  const { error: itemUpdateError } = await supabase
    .from("prestamos_detalle")
    .update({ estado_item: "prestado", fecha_resolucion: null })
    .eq("id", idDetalleItem);

  if (itemUpdateError) throw itemUpdateError;

  // El préstamo vuelve a tener algo prestado, así que queda activo
  const { error: prestamoError } = await supabase
    .from("prestamos")
    .update({ estado: "activo", updated_at: new Date().toISOString() })
    .eq("id", item.id_prestamo);
...
// Corrige los datos generales de un préstamo (fecha, distribuidora,
// notas). No toca los ítems ni el stock - eso se maneja aparte con
// agregarJoyaAPrestamo / quitarJoyaDePrestamo / revertirItemPrestamo.
