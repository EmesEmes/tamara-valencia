...
export default function ConjuntosGrid() {
  ...
  const [cantidadMostrada, setCantidadMostrada] =
    useState(CONJUNTOS_POR_PAGINA);
  ...
  const fetchConjuntos = async () => {
    try {
      ...
      setConjuntosMostrados(
        conjuntosConProductos.slice(0, CONJUNTOS_POR_PAGINA),
      );
    } catch (error) {
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-500">
                    {conjunto.productos[0]?.count || 0}{" "}
                    {conjunto.productos[0]?.count === 1 ? "pieza" : "piezas"}
                  </p>
                </div>
        ...
        {hayMasConjuntos && (
          <div className="text-center mt-12">
            ...
            >
              {loadingMore
                ? "Cargando..."
                : `Ver Más Colecciones (${conjuntos.length - cantidadMostrada} restantes)`}
            </button>
          </div>
        )}
